...
//                }
    ...
    public ImageContainer get(String requestUrl, ImageListener imageListener,
        ...
        BatchedImageRequest batchedImageRequest = mInFlightRequests.get(cacheKey);
    ...
    private Request makeImageRequest(String requestUrl, int maxWidth, int maxHeight,
    ...
    protected void onGetImageSuccess(String cacheKey, Bitmap response) {
        ...
        if (SPHelper.create().isDiskCacheEnable()) {
            mDiskCache.put(cacheKey, response);
        }
     ...
     * @param cacheKey The cache key that is associated with the image request.
     */
    protected void onGetImageError(String cacheKey, Exception error) {
                ...
                public void run() {
                    for (BatchedImageRequest bir : mBatchedResponses.values()) {
                        for (ImageContainer container : bir.mContainers) {
                            ...
                            // skip them.
                            if (container.getImageListener() == null) {
                                continue;
                            }
                            if (bir.getError() == null) {
                                container.mBitmap = bir.mResponseBitmap;
                                container.getImageListener().onResponse(container, false);
                            } else {
                                container.getImageListener().onErrorResponse(bir.getError());
                            }
                        }
                    }
    ...
    public class ImageContainer {
        ...
        private final WeakReference<ImageListener> callback;
         ...
         */
        ImageContainer(Bitmap bitmap, String requestUrl,
                       String cacheKey, ImageListener listener) {
            mBitmap = bitmap;
            mRequestUrl = requestUrl;
            mCacheKey = cacheKey;
//            mListener = listener;
            callback = new WeakReference<>(listener);
        }
